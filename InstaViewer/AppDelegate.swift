...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let mainViewController = SocialFeedViewController()
        let navigationController = UINavigationController(rootViewController: mainViewController)
        window?.rootViewController = navigationController
        window?.makeKeyAndVisible()
        
        // Theme customization after application launch.
        Chameleon.setGlobalThemeUsingPrimaryColor(UIColor.flatMint, with: .contrast)
        
        // Realm DB configuration
        do {
           var dbConfig = try Realm().configuration
            dbConfig.deleteRealmIfMigrationNeeded = true
